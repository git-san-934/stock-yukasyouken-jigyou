...
function render(term){{
  ...
  const hits=DATA.filter(c=>!term||c.name.toLowerCase().includes(term)
     ||c.code.toLowerCase().includes(term)||c.body.toLowerCase().includes(term));
  count.textContent=hits.length+' 件';
  list.innerHTML=hits.map(c=>{{
    let snip='';
    if(term){{const i=c.body.toLowerCase().indexOf(term);
